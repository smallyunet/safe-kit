...
from safe_kit.abis import SAFE_ABI, SAFE_PROXY_FACTORY_ABI, ERC20_ABI, ERC721_ABI
...
def process_abi(abi: List[Dict[str, Any]], prefix: str) -> str:
    ...
    for item in abi:
        ...
        # For now, we'll just skip if we've seen it, which isn't perfect but works for simple cases
        ...
        # If outputs are named, we might want a TypedDict, but Web3.py returns tuples/values, not dicts.
        # So generating TypedDict for return values might be misleading unless we wrap the return.
        # For now, let's stick to Params which are useful for calling functions.
        
    return output

def main():
    output_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "safe_kit", "contract_types.py"))
